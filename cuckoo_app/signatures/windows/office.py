...
class OfficeCreateObject(Signature):
    ...
    #}
    descriptions = {
        "network": u"或有尝试连接外部网络",
        "file": u"尝试在硬盘中写入一个或者多个文件",
        "process": u"可能尝试创建新的进程文件",
    }
...
class OfficeCheckProjectName(Signature):
    ...
    description = u"Office 检查 VB 项目名称"
    ttp = ["T1059.005"]
    severity = 1
    categories = ["vba"]
...
class OfficeCountDirectories(Signature):
    ...
    ttp = ["T1497"]
    description = u"office文档调用（可能的）反沙箱"
    severity = 2
    categories = ["vba"]
...
class OfficeCheckVersion(Signature):
    ...
    description = u"office文档检测office版本（可能反沙箱）"
    severity = 2
    categories = ["vba"]
    authors = ["FDD", "Cuckoo Technologies"]
    minimum = "2.0"
...
class OfficeCheckWindow(Signature):
    ...
    description = u"office文档检测office窗口大小（可能反沙箱）"
    severity = 2
    categories = ["vba"]
    authors = ["FDD @ Cuckoo Technologies"]
    minimum = "2.0"
...
class OfficeHttpRequest(Signature):
    ...
    description = u"office文档执行Http请求（可能下载恶意软件）"
    severity = 5
    categories = ["vba"]
    authors = ["Cuckoo Technologies"]
    minimum = "2.0"
...
class OfficeRecentFiles(Signature):
    ...
    description = u"执行RecentFiles确定是否在沙箱运行"
    severity = 4
    categories = ["vba"]
    authors = ["Cuckoo Technologies"]
    minimum = "2.0"
...
class HasOfficeEps(Signature):
    ...
    description = u"位于潜在恶意封装的后脚本（EPS） 文档"
    severity = 3
    categories = ["office"]
    authors = ["Cuckoo Technologies"]
    minimum = "2.0"
...
class OfficeCheckName(Signature):
    ...
    description = u"Office 文档检查其自己的名称"
    severity = 2
    categories = ["office"]
    authors = ["FDD", "Cuckoo Technologies"]
    minimum = "2.0"
...
class OfficeEpsStrings(Signature):
    ...
    description = u"嵌入在封装的后脚本 （EPS） 文档中的可疑关键字"
    severity = 3
    categories = ["office"]
    authors = ["Cuckoo Technologies"]
    minimum = "2.0"
...
class OfficeVulnerableGuid(Signature):
    ...
    description = u"请求了已知与 CVE 关联的 GUID（可能是误报）"
    severity = 3
    categories = ["office"]
...
class OfficeVulnModules(Signature):
    ...
    description = u"请求已知与 CVE 关联的库（可能是误报）"
    severity = 3
    categories = ["office"]
    authors = ["Niels Warnars @ Cuckoo Technologies"]
    minimum = "2.0"
