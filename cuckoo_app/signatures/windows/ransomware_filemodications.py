...
class RamsomwareFileMoves(Signature):
    ...
    description = u"执行文件移动指示显示Ransomware文件加密过程"
    severity = 3
...
class RansomwareAppendsExtension(Signature):
    ...
    description = u"追加新的文件扩展名以及内容显示Ransomware文件加密过程"
    severity = 3
    categories = ["ransomware"]
    authors = ["Kevin Ross"]
    minimum = "2.0"
...
class RansomwareDroppedFiles(Signature):
    ...
    description = u"删除未知文件mime类型说明Ransomware将加密文件写入磁盘"
    severity = 3
    categories = ["ransomware"]
    authors = ["Kevin Ross"]
    minimum = "2.0"
...
class RansomwareMassFileDelete(Signature):
    ...
    description = u"删除大量勒索软件、wiper malware或者系统破坏软件"
    severity = 3
    ttp = ["T1070.004"]
