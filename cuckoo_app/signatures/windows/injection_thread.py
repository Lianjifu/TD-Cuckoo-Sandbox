...
class InjectionCreateRemoteThread(Signature):
    ...
    description = u"恶意使用CreateRemote Thread创建线程可能进行进程注入"
    severity = 3
    ttp = ["T1055"]
...
class InjectionQueueApcThread(Signature):
    ...
    description = u"在远程进程中使用 NtQueueApcThread 创建线程，可能指示进程注入"
    ttp = ["T1055"]
    severity = 3
    categories = ["injection"]
    authors = ["Kevin Ross"]
...
class ResumeThread(Signature):
    ...
    ttp = ["T1055"]
    description = u"恢复了远程进程中的挂起线程，可能指示进程注入"
    severity = 3
    categories = ["injection"]
    authors = ["Kevin Ross"]
    minimum = "2.0"
...
class NtSetContextThreadRemote(Signature):
    ...
    description = u"使用 NtSetContextThread 修改远程进程中指示进程注入的线程"
    severity = 3
