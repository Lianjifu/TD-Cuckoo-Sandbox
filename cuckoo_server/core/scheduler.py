...
class AnalysisManager(threading.Thread):
    ...
    def build_options(self):
        ...
        if self.task.category == "file":
            ...
            if "Zip" in File(self.task.target).get_type():
                options["file_type"] = self.trid_file_type()
            else:
    ...
    def launch_analysis(self):
        ...
        try:
            ...
            # start analog USB flash drive insertion
            #machinery.usb_drive_insertion_control(self.machine.label)
            ...
            if "noagent" not in self.machine.options:
                ...
                machinery.screenshot_virtual_machine(self.machine.label,self.task.id) 
            else:
                self.wait_finish()

            # start analog USB flash drive removal
            #machinery.usb_drive_removal_control(self.machine.label)
            succeeded = True
    ...
    def trid_file_type(self):
        """trid file type 
        @param file_path: file path.
        @return: file type.
        """
        trid_cmdline = "export LC_CTYPE=C.UTF-8 && /opt/trid/trid {0}".format(self.task.target)
        try:
            proc = subprocess.Popen(trid_cmdline, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            res = proc.stdout.read().decode("utf-8")
            file_type = str(res).split("%")[1].split("\n")[0]
            if ".DOC" in str(file_type):
                return "Microsoft Office Word"
            elif ".XLS" in str(file_type):
                return "Microsoft Office Excel"
            elif ".PPT" in str(file_type):
                return "Microsoft Office PowerPoint"
            else:
