...
class Machinery(object):
    ...
    def usb_drive_insertion_control(self, label):
        """Analog USB flash drive insertion.
        @param label: machine name.
        @return: True/False
        """
        raise NotImplementedError

    def usb_drive_removal_control(self, label):
        """Analog USB flash drive removal.
        @param label: machine name.
        @return: True/False
        """
        raise NotImplementedError

    def screenshot_virtual_machine(self,label,task):
...
class LibVirtMachinery(Machinery):
    ...
    def start(self, label, task):
        ...
        # Analog USB flash drive insertion
        #self.usb_drive_insertion_control(label)
    ...
    def usb_drive_insertion_control(self, label):
        ...
        log.info("Starting analog USB flash drive insertion for {}".format(label))
        usb_attach_path = '/home/ywadmin/kvm/usb.xml'
        with open(usb_attach_path,'r') as f:
            usb_attach_xml = f.read()
        try:
            self.vms[label].attachDevice(usb_attach_xml)
            log.info("analog USB flash drive insertion successfully for {0}".format(label))
            return True
        except OSError as e:
            log.debug("cmd analog USB flash drive failed: %s" % e)
        
    def usb_drive_removal_control(self, label):
        """
        Analog USB flash drive removal
        """
        log.info("Starting analog USB flash drive removal for {}".format(label))
        usb_attach_path = '/home/ywadmin/kvm/usb.xml'
        with open(usb_attach_path,'r') as f:
            usb_attach_xml = f.read()
        try:
            self.vms[label].detachDevice(usb_attach_xml)
            log.info("analog USB flash drive removal successfully for {0}".format(label))
            return True
        except OSError as e:
            log.dedug("cmd analog USB flash drive failed: %s" % e)

    def screenshot_virtual_machine(self,label,task):
        """
        Screenshot of the virtual machine.
        """
        log.info("Starting screenshot of the virtual machine for {}".format(label))
        screenshot_path = '/home/ywadmin/.cuckoo/storage/analyses/{0}/shots/00A.jpg'.format(task)
        screenshot_cmdline = "echo Yw@qazWSX | sudo -S virsh screenshot {0} {1}".format(label,screenshot_path)
        try:
            proc = subprocess.Popen(screenshot_cmdline,shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            log.info("out:::{}".format(proc.stdout.readlines()))
            log.info("err:::{}".format(proc.stderr.readlines()))
            log.info("screenshot of the virtual machine successfully for {0}".format(label))
            return True
        except OSError as e:
            log.dedug("screenshot of the virtual machine failed: %s" % str(e))
