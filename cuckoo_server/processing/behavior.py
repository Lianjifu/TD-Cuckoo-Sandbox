...
class BehaviorAnalysis(Processing):
    ...
    def run(self):
        ...
        log.info("-----1-----{}-------------".format(str(time.time() - ttt)))
                    ...
                    for hhandler in interest_map.get(event["type"], []):
                        ...
                        for subevent in res:
                            for hhandler2 in interest_map.get(subevent["type"], []):
                                hhandler2(subevent)
                log.info("{}{}{}".format(handler,path,str(time.time() - t)))
        log.info("-----222-----{}-------------".format(str(time.time() - ttt)))
        behavior = {}
        ...
        for handler in handlers:
            try:
                t = time.time()
                r = handler.run()
                log.info("----------{}-------------{}".format(handler, str(time.time() - t)))
                if not r:
                    continue
